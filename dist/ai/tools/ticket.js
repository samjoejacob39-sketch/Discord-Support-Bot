import { argString } from './types.js';
/**
 ...
 */
export const createTicketSummary = {
    spec: {
        name: 'create_ticket_summary',
        description: 'Save a short factual summary of this conversation so far. Useful in long tickets so context survives without resending every message. Facts only, no advice, no secrets.',
        parameters: {
            type: 'object',
            properties: {
    ...
    async run(args, ctx) {
        ...
        if (summary.length < 20)
            return { error: 'Summary too short to be useful.' };
...
export const resumeTicket = {
    spec: {
        name: 'resume_ticket',
        description: 'Staff-only: hand a ticket that is waiting for a human back to AI handling, keeping its context. Equivalent to /shin-continue.',
        parameters: {
            type: 'object',
            properties: {
    ...
    async run(args, ctx) {
        if (!ctx.askerIsStaff)
            return { error: 'Only Shinchat Helper admins can resume a ticket.' };
...
export const closeTicket = {
    ...
    async run(_args, ctx) {
        if (!ctx.askerIsStaff)
            return { error: 'Only Shinchat Helper admins can close a ticket.' };
...
export const ticketTools = [createTicketSummary, resumeTicket, closeTicket];
