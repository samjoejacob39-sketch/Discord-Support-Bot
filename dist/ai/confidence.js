...
export function parseConfidence(value, fallback = 'medium') {
    if (typeof value !== 'string')
        return fallback;
    const normalized = value.trim().toLowerCase();
    return CONFIDENCE_LEVELS.includes(normalized) ? normalized : fallback;
}
...
export function requestsHuman(text) {
...
export function looksFrustrated(text) {
...
export function judgeAfterAnswer(input) {
    if (requestsHuman(input.userText))
        return { escalate: true, reason: 'The member asked to speak to a human.' };
