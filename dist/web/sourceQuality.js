...
export function scoreResult(result, query = '', index = 0) {
    ...
    if (OUTAGE_INTENT.test(query) && /status|incident/i.test(`${result.host}${result.url}`))
        score += 25;
    if (VERSION_INTENT.test(query) && /releases?|changelog|tags/i.test(result.url))
        score += 20;
...
//# sourceMappingURL=sourceQuality.js.map