...
export class DuckDuckGoSearchProvider {
    ...
    async search(query, options = {}) {
        ...
        if (options.recency === 'day')
            url.searchParams.set('df', 'd');
        else if (options.recency === 'week')
            url.searchParams.set('df', 'w');
        else if (options.recency === 'month')
            url.searchParams.set('df', 'm');
        const response = await withTimeout(fetch(url, { headers: { 'User-Agent': 'ShinchatHelper/1.0 (+support bot)' } }), TIMINGS.webSearchTimeoutMs);
        if (!response.ok)
