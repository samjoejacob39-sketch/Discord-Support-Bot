...
const LIST_STATES = ['WAITING_FOR_ADMIN', 'ADMIN_ACTIVE', 'AI_ACTIVE', 'NEW', 'AI_PAUSED', 'RESOLVED'];
const ticketOption = (description) => (option) => option.setName('ticket').setDescription(description).setMinValue(1);
const data = new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('reason').setDescription('Why staff need to take over.').setMaxLength(500))
    .addIntegerOption(ticketOption('Ticket id.')))
    .addSubcommand((sub) => sub
    .setName('resolve')
...
async function runList({ interaction, ctx }) {
    const state = interaction.options.getString('state') ?? 'WAITING_FOR_ADMIN';
    const tickets = ctx.store.tickets.listByState(interaction.guildId, state, 10);
    await interaction.reply({
        embeds: [
            infoEmbed(`Tickets · ${STATE_LABELS[state]}`, tickets.length > 0
...
async function runSummary(invocation, ticket) {
...
async function runEscalate(invocation, ticket) {
...
export const ticketCommand = {
    ...
    async execute(invocation) {
        ...
        if (!ticket) {
            await interaction.reply({
                embeds: [
                    errorEmbed('No ticket found', 'Run this in the ticket’s channel, or pass `ticket:<id>`. `/ticket list` shows the ids.'),
                ],
                flags: MessageFlags.Ephemeral,
            });
            return;
        ...
        if (sub === 'summary')
            return runSummary(invocation, ticket);
        if (sub === 'escalate')
            return runEscalate(invocation, ticket);
        if (sub === 'ai') {
            ...
            await interaction.reply({
                embeds: [
                    moved
                        ? successEmbed(enabled ? 'AI resumed here' : 'AI paused here', enabled
        ...
        await interaction.reply({
            embeds: [
                moved
                    ? successEmbed(closing ? 'Ticket closed' : 'Marked resolved', closing
