...
const data = new SlashCommandBuilder()
    ...
    .addChannelOption((option) => option.setName('channel').setDescription('Staff channel.').addChannelTypes(...TEXT_CHANNEL_TYPES)))
    .addSubcommand((sub) => sub
    ...
    .addIntegerOption((option) => option.setName('value').setDescription('1–5 attempts.').setRequired(true).setMinValue(1).setMaxValue(5)))
    .addSubcommand((sub) => sub
    .setName('persona')
    .setDescription('Extra tone guidance for replies. Leave empty to clear.')
    .addStringOption((option) => option.setName('note').setDescription('E.g. "Be brief and mention our EU hours."').setMaxLength(MAX_PERSONA_NOTE)))
    .addSubcommand((sub) => sub.setName('reset').setDescription('Restore every setting to its default.'))
    .toJSON();
function describe(settings) {
    const list = (ids, render) => ids.length > 0 ? ids.map(render).join(', ') : '_none_';
    return [
...
async function apply(invocation, patch, title, detail) {
...
async function editSurface(invocation, kind, id, remove) {
    ...
    if (remove) {
        ...
        await apply(invocation, kind === 'channel' ? { supportChannelIds: next } : { supportCategoryIds: next }, `Support ${kind} removed`, `${mention} is no longer a support ${kind}.`);
        return;
    }
    if (current.includes(id)) {
        await interaction.reply({
    ...
    const hint = kind === 'channel' && settings.supportMode !== 'channels'
        ...
        : kind === 'category' && settings.supportMode !== 'categories'
            ...
            : '';
    await apply(invocation, kind === 'channel' ? { supportChannelIds: next } : { supportCategoryIds: next }, `Support ${kind} added`, `${mention} is now a support ${kind}.${hint}`);
}
/**
 ...
 */
export const shinconfigCommand = {
    name: 'shinconfig',
    ...
    async execute(invocation) {
        ...
        if (sub === 'view') {
            await interaction.reply({
                embeds: [
                    ...
                    infoEmbed('Support modes', Object.entries(SUPPORT_MODE_HELP)
        ...
        if (sub === 'mode') {
            ...
            const missing = (mode === 'channels' && settings.supportChannelIds.length === 0) ||
                (mode === 'categories' && settings.supportCategoryIds.length === 0);
            await apply(invocation, { supportMode: mode }, 'Support mode updated', [
        ...
        if (sub === 'escalation-channel') {
            ...
            await apply(invocation, { escalationChannelId: channel?.id ?? null }, channel ? 'Escalation channel set' : 'Escalation channel cleared', channel
        ...
        if (sub === 'trusted-role') {
            ...
            await apply(invocation, { trustedRoleId: role?.id ?? null }, role ? 'Trusted role set' : 'Trusted role cleared', role
        ...
        if (sub === 'ping-role') {
            const role = interaction.options.getRole('role');
            await apply(invocation, { adminPingRoleId: role?.id ?? null }, role ? 'Ping role set' : 'Ping role cleared', role ? `<@&${role.id}> will be pinged with every escalation.` : 'Only the listed admins will be pinged.');
            return;
        }
        if (sub === 'ai') {
            const enabled = interaction.options.getBoolean('enabled', true);
            await apply(invocation, { aiEnabled: enabled }, enabled ? 'AI answering enabled' : 'AI answering disabled', enabled
        ...
        if (sub === 'attempts') {
            ...
            await apply(invocation, { maxAiAttempts: value }, 'Attempt limit updated', `After ${value} unsuccessful ${value === 1 ? 'try' : 'tries'} in one ticket I hand it to a human instead of guessing again.`);
            return;
        }
        if (sub === 'persona') {
            const note = interaction.options.getString('note')?.trim();
            await apply(invocation, { personaNote: note && note.length > 0 ? note : null }, note ? 'Persona note saved' : 'Persona note cleared', note
...
//# sourceMappingURL=shinconfig.js.map