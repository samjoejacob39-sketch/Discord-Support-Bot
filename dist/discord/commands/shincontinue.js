...
const data = new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('note').setDescription('Optional context for the AI, e.g. what you already fixed.').setMaxLength(500))
    .addIntegerOption((option) => option.setName('ticket').setDescription('Ticket id, if you are not in its channel.').setMinValue(1))
    .toJSON();
/** `/shin-continue` — the only way out of WAITING_FOR_ADMIN (§11). */
export const shinContinueCommand = {
    name: 'shin-continue',
    access: 'shin_admin',
    category: 'tickets',
    ...
    async execute({ interaction, ctx }) {
        ...
        if (!ticket) {
            await interaction.reply({
                embeds: [
                    errorEmbed('No ticket here', 'Run this in the ticket’s channel, or pass `ticket:<id>` (find ids with `/ticket status`).'),
                ],
        ...
        await interaction.reply({
            embeds: [
                successEmbed('AI support resumed', [
...
//# sourceMappingURL=shincontinue.js.map