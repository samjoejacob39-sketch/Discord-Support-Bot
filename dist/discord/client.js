...
export function attachHandlers(client, ctx) {
    client.once(Events.ClientReady, (ready) => {
        log.info({ user: ready.user.tag, guilds: ready.guilds.cache.size, provider: ctx.provider.name }, 'Shinchat Helper is online');
    ...
    client.on(Events.InteractionCreate, (interaction) => {
        void handleInteraction(interaction, ctx).catch((error) => log.error({ err: errorMessage(error) }, 'interaction handler crashed'));
    });
    client.on(Events.MessageCreate, (message) => {
        void handleMessage(message, ctx).catch((error) => log.error({ err: errorMessage(error) }, 'message handler crashed'));
    });
    client.on(Events.GuildCreate, (guild) => {
        void handleGuildCreate(guild, ctx).catch((error) => log.error({ guildId: guild.id, err: errorMessage(error) }, 'guild join handler crashed'));
    });
