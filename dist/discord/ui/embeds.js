...
export function baseEmbed(color, title) {
    ...
    if (title)
        embed.setTitle(truncate(title, 250));
    return embed;
}
export function infoEmbed(title, description) {
    const embed = baseEmbed(COLORS.primary, title);
    if (description)
        embed.setDescription(truncate(description, DISCORD_LIMITS.embedDescription));
    return embed;
}
...
export function knowledgeEmbed(entry) {
    const embed = baseEmbed(entry.kind === 'incident' ? COLORS.danger : COLORS.primary, `#${entry.id} · ${entry.title}`)
        ...
        .addFields({ name: 'Category', value: categoryLabel(entry.category), inline: true }, { name: 'Kind', value: entry.kind, inline: true }, { name: 'Status', value: entry.status, inline: true }, { name: 'Visibility', value: entry.visibility === 'staff' ? '🔒 staff only' : 'public', inline: true }, { name: 'Taught by', value: `<@${entry.createdBy}>`, inline: true }, { name: 'Added', value: relativeTimestamp(entry.createdAt), inline: true });
    if (entry.expiresAt) {
    ...
    if (entry.flagged) {
        embed.addFields({
            ...
            value: 'This note contains phrasing that looks like an instruction override attempt. It is stored as **policy data only** and can never change the bot’s safety rules.',
        });
    }
    return embed;
}
export function knowledgeListEmbed(page) {
...
export function ticketEmbed(ticket, extras = {}) {
    const embed = baseEmbed(COLORS.primary, `Ticket #${ticket.id}`)
        ...
        .addFields({ name: 'Opened by', value: `<@${ticket.openerUserId}>`, inline: true }, { name: 'Channel', value: `<#${ticket.channelId}>`, inline: true }, { name: 'Started', value: relativeTimestamp(ticket.createdAt), inline: true }, { name: 'AI attempts', value: String(ticket.aiAttempts), inline: true }, { name: 'Escalations', value: String(ticket.escalationCount), inline: true }, { name: 'Last activity', value: relativeTimestamp(ticket.lastActivityAt), inline: true });
    if (ticket.subject)
        embed.addFields({ name: 'Topic', value: truncate(ticket.subject, 200) });
