...
import type { TicketContext } from '../tickets/service.js';
import { STATE_LABELS } from '../tickets/stateMachine.js';
import type { AITurn } from './provider.js';
...
export interface PromptInput {
  guildName: string;
  /** Admin-authored persona/tone note. Treated as server policy, never as a rule override. */
  ...
  /** True when the person talking is a staff member (changes what may be shared). */
  askerIsStaff: boolean;
  /** Rendered by `renderKnowledgeBlocks`. */
...
function situationBlock(input: PromptInput): string {
  const lines = [
  ...
  if (input.ticket) {
    ...
    if (input.ticket.subject) lines.push(`Topic so far: ${input.ticket.subject}`);
  }
  if (typeof input.attemptsUsed === 'number' && typeof input.maxAttempts === 'number') {
    lines.push(
      `AI attempts on this ticket: ${input.attemptsUsed}/${input.maxAttempts}` +
  ...
  if (!input.webFetchAvailable) lines.push('Page fetching: disabled.');
  return dataBlock('situation', lines.join('\n'));
}

/** Assemble the full system instruction for one AI turn. */
export function buildSystemPrompt(input: PromptInput): string {
  const parts: string[] = [SAFETY_RULES, KNOWLEDGE_HIERARCHY, STYLE_RULES, CONFIDENCE_RULES, TOOL_CONTRACT];

  parts.push(situationBlock(input));

  if (input.personaNote && input.personaNote.trim().length > 0) {
    parts.push(
      envelope('server_policy', input.personaNote, {
  ...
  for (const block of input.knowledgeBlocks ?? []) parts.push(block);
  for (const block of input.contextBlocks ?? []) parts.push(block);

  return parts.join('\n\n');
}

/** Ticket facts and the rolling summary, as prompt blocks. */
export function renderTicketContextBlocks(context: TicketContext): string[] {
  const blocks: string[] = [];
  if (context.summary) {
    blocks.push(
      dataBlock('conversation_summary', context.summary.summary, {
  ...
  if (context.facts.length > 0) {
    blocks.push(
      dataBlock('ticket_facts', context.facts.map((fact) => `- ${fact.label}: ${fact.value}`).join('\n'), {
        maxChars: 1200,
      }),
    );
  }
...
const AUTHOR_LABEL: Record<string, string> = { user: 'member', bot: BOT_NAME, admin: 'staff' };
 ...
 */
export function buildTurns(context: TicketContext, latestQuestion: string, latestAuthorId: string): AITurn[] {
  const turns: AITurn[] = [];
  ...
  const alreadyLast =
    context.messages.length > 0 && context.messages[context.messages.length - 1]?.content.trim() === latestQuestion.trim();
  if (!alreadyLast && latestQuestion.trim().length > 0) {
    pendingUser.push(
      envelope('user_content', latestQuestion, {
