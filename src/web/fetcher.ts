...
export function isBlockedAddress(address: string): boolean {
  ...
  if (/^(64:ff9b|2002:|2001:0?db8)/.test(normalized)) return true; // translation/doc ranges
  return false;
}
...
export async function assertResolvesPublicly(url: URL): Promise<void> {
  if (isIP(url.hostname) !== 0) {
    if (isBlockedAddress(url.hostname)) throw new UnsafeUrlError('That address is not publicly routable.');
    return;
  }
