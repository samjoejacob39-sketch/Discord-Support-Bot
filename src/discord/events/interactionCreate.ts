...
async function fail(interaction: ChatInputCommandInteraction, title: string, detail: string): Promise<void> {
...
export async function handleInteraction(interaction: Interaction, ctx: BotContext): Promise<void> {
  ...
  if (!interaction.inCachedGuild()) {
    await fail(
      interaction,
      'Server only',
      'Shinchat Helper works inside a server, not in direct messages. Use the commands in your community.',
    );
    return;
  }
  ...
  try {
    ...
    await fail(
      interaction,
      'That did not work',
      'Something broke on my side. Nothing was changed. Please try again — if it keeps failing, tell a server manager.',
    );
  }
}
