import { ChannelType, PermissionFlagsBits, type Guild, type GuildTextBasedChannel } from 'discord.js';
...
function greetingChannel(guild: Guild): GuildTextBasedChannel | undefined {
...
export async function handleGuildCreate(guild: Guild, ctx: BotContext): Promise<void> {
  ...
  await channel
    ...
    .catch((error: unknown) => log.warn({ guildId: guild.id, err: String(error) }, 'greeting failed'));
}

/** Keep the row for history and knowledge, but mark the guild as left. */
export function handleGuildDelete(guild: Guild, ctx: BotContext): void {
