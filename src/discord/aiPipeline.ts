...
/** How the answer reaches the member: a channel message, or an interaction reply. */
export type Deliver = (content: string, index: number) => Promise<string | undefined>;
...
export interface SupportTurnInput {
  ...
  deliver: Deliver;
  /** Skip the shared rate limiter (already applied by the caller). */
  skipRateLimit?: boolean;
}

export type SupportTurnResult =
  | { status: 'answered'; ticket: Ticket; message: string; cached?: boolean }
  | { status: 'escalated'; ticket: Ticket; message: string }
  | { status: 'silent'; ticket: Ticket; reason: string }
  | { status: 'rate_limited'; retryAfterMs: number; scope: 'user' | 'guild' };
 ...
 */
export async function runSupportTurn(input: SupportTurnInput): Promise<SupportTurnResult> {
  ...
  if (outcome.kind === 'answer') {
    ...
    if (outcome.confidence === 'high') ctx.tickets.markAiSuccess(active);
    void ctx.summarizer.maybeSummarise(guild.id, active.id).catch(() => undefined);
    log.debug(
      { guildId: guild.id, ticketId: active.id, confidence: outcome.confidence, tools: outcome.toolCalls.length },
      'answered',
    );
...
async function deliverAll(input: SupportTurnInput, content: string): Promise<void> {
