...
export const shinstatusCommand: CommandModule = {
  name: 'shinstatus',
  access: 'shin_admin',
  category: 'config',
  ...
  async execute({ interaction, ctx, settings }) {
    ...
    const health = await ctx.provider.health().catch((error: unknown) => ({ ok: false, detail: String(error) }));
    ...
    const ticketLines = Object.entries(states)
      ...
      .map(([state, count]) => `${STATE_LABELS[state as keyof typeof STATE_LABELS] ?? state} — ${count}`)
      .join('\n');

    const escalationRate =
      usage.requests > 0 ? `${Math.round((usage.escalations / usage.requests) * 100)}%` : 'n/a';
