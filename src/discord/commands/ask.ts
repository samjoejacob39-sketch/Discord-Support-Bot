...
const data = new SlashCommandBuilder()
  ...
  .addBooleanOption((option) =>
    option.setName('private').setDescription('Answer only to you (default: visible to the channel).'),
  )
  .toJSON();
  ...
  async execute({ interaction, ctx, member, settings }) {
    ...
    if (result.status === 'silent') {
      await interaction.editReply({
        embeds: [
          warningEmbed(
            'A human is handling this',
            'This conversation is with the staff team right now, so I am staying out of it. An admin can hand it back to me with `/shin-continue`.',
          ),
        ],
      });
    }
