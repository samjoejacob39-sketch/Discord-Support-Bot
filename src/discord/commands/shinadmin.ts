 ...
 */
export const shinadminCommand: CommandModule = {
  name: 'shinadmin',
  ...
  async execute({ interaction, ctx, settings }) {
    ...
    if (sub === 'list') {
      ...
      const lines = admins.map(
        (admin) => `• <@${admin.userId}> — added by <@${admin.addedBy}> ${relativeTimestamp(admin.addedAt)}`,
      );
      ...
      await interaction.reply({
        embeds: [
          added
            ? successEmbed(
                'Admin added',
                `<@${target.id}> can now use \`/learn\`, \`/knowledge\`, \`/ticket\` and \`/shin-continue\`, and will be pinged on escalations.`,
              )
    ...
    if (removed) ctx.store.audit.record(guildId, interaction.user.id, AUDIT_ACTIONS.adminRemove, target.id);
    ...
    await interaction.reply({
      embeds: [
        removed
          ? successEmbed(
              'Admin removed',
              `<@${target.id}> no longer has bot admin rights.` +
