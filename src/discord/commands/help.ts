...
import type { CommandCategory, CommandModule } from './types.js';

const RANK: Record<AccessLevel, number> = { member: 0, shin_admin: 1, guild_manager: 2 };
...
const CATEGORY_ORDER: CommandCategory[] = ['general', 'knowledge', 'tickets', 'config'];
...
const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option.setName('command').setDescription('Show details for one command.').setAutocomplete(true),
  )
  .toJSON();

function visible(level: AccessLevel, command: CommandModule): boolean {
 ...
 */
export function createHelpCommand(all: () => CommandModule[]): CommandModule {
  return {
    name: 'help',
    access: 'member',
    category: 'general',
    ...
    async execute({ interaction, level }) {
      ...
      if (requested) {
        ...
        if (!command || !visible(level, command)) {
          await interaction.reply({
            embeds: [
              infoEmbed(
                'No such command for you',
                `I have no \`/${requested}\` you can use. Run \`/help\` to see your list.`,
              ),
            ],
      ...
      for (const category of CATEGORY_ORDER) {
        ...
        if (lines.length > 0) embed.addFields({ name: CATEGORY_TITLES[category], value: lines.join('\n') });
      }
