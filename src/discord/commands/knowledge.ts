...
import type { KnowledgeKind, KnowledgeStatus } from '../../db/types.js';
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((option) =>
        option.setName('category').setDescription('Only this category.').addChoices(...categoryChoices().slice(0, 25)),
      )
      .addStringOption((option) =>
        option
          .setName('kind')
...
async function runList({ interaction, ctx }: CommandInvocation): Promise<void> {
  ...
  const kind = (interaction.options.getString('kind') as KnowledgeKind | null) ?? undefined;
  const status = (interaction.options.getString('status') as KnowledgeStatus | 'any' | null) ?? undefined;
...
async function runSearch({ interaction, ctx }: CommandInvocation): Promise<void> {
...
export const knowledgeCommand: CommandModule = {
  ...
  async execute(invocation) {
    ...
    if (sub === 'search') return runSearch(invocation);
    ...
    await interaction.reply({
      embeds: [
        successEmbed(
          ...
          `\`#${id}\` **${entry.title}** is now ${enable ? 'active again' : 'ignored, but kept'}.`,
        ),
      ],
      flags: MessageFlags.Ephemeral,
    });
