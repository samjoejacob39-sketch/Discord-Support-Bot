...
async function main(): Promise<void> {
  ...
  log.info(
    { count: body.length, scope: env.DISCORD_DEV_GUILD_ID ? `guild ${env.DISCORD_DEV_GUILD_ID}` : 'global' },
    'commands registered',
  );
  for (const command of COMMANDS) log.info(`  /${command.name} — ${command.summary}`);
  if (!env.DISCORD_DEV_GUILD_ID) log.info('Global commands can take up to an hour to appear everywhere.');
}
...
main().catch((error: unknown) => {
  ...
  if (status === 401) log.fatal('401 means DISCORD_TOKEN is wrong or was reset. Copy it again from the Bot tab.');
  if (status === 404) log.fatal('404 usually means DISCORD_APPLICATION_ID is wrong, or the bot is not in that guild.');
  finishExit(1);
});
