...
export interface RetrievalResult {
  incidents: KnowledgeEntry[];
  matches: KnowledgeEntry[];
  /** Every entry actually placed in the prompt, for the escalation summary. */
  used: KnowledgeEntry[];
}
...
export function retrieveKnowledge(
  ...
  guildId: string,
  query: string,
  options: { matchLimit?: number; charBudget?: number } = {},
): RetrievalResult {
  ...
  const seen = new Set<number>(incidents.map((entry) => entry.id));
  ...
  for (const entry of [...incidents, ...uniqueMatches]) {
    ...
    if (spent + cost > budget && used.length > 0) break;
    used.push(entry);
    spent += cost;
  }
...
function renderEntry(entry: KnowledgeEntry): string {
  const flags: string[] = [`id=${entry.id}`, `category=${entry.category}`];
  if (entry.kind !== 'permanent') flags.push(`kind=${entry.kind}`);
  if (entry.visibility === 'staff') flags.push('staff_only=true');
  if (entry.expiresAt) flags.push(`expires_at=${new Date(entry.expiresAt).toISOString()}`);
 ...
 */
export function renderKnowledgeBlocks(result: RetrievalResult): string[] {
  const blocks: string[] = [];

  if (result.incidents.length > 0) {
    blocks.push(
      dataBlock('active_incidents', result.incidents.map(renderEntry).join('\n\n'), {
  ...
  if (general.length > 0) {
    blocks.push(
      dataBlock('server_knowledge', general.map(renderEntry).join('\n\n'), {
...
export function summariseEntries(entries: KnowledgeEntry[]): string {
  if (entries.length === 0) return '_Nothing yet._';
  return entries
    .map((entry) => {
      const badges = [
