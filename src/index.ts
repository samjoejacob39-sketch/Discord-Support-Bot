...
async function main(): Promise<void> {
  ...
  if (errors.length > 0) {
    for (const error of errors) log.fatal(error);
  ...
  const shutdown = (signal: string): void => {
    ...
    void client
      .destroy()
      .catch((error: unknown) => log.warn({ err: errorMessage(error) }, 'client shutdown failed'))
      .finally(() => {
        closeDatabase();
        finishExit(0);
      });
  ...
  try {
    ...
    log.fatal(
      ...
      'could not sign in to Discord — check DISCORD_TOKEN in .env (Developer Portal → Bot → Reset Token)',
    );
    scheduler.stop();
